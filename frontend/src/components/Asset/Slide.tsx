  ...
  return (
    <div className="">
      <div className="flex pb-8 gap-10 ">
        <h3
          id={id}
          className="scroll-mt-8 text-2xl font-bold inline-flex bg-clip-text text-transparent bg-gradient-to-r from-slate-200/60 via-slate-200 to-slate-200/60 "
        >
          {title ? title : id}
        </h3>
        {/* <SeeAll /> */}
      ...
      <div className={`${id}-carousel swiper-container group`}>
        <div className="swiper-wrapper w-fit">
          {items.map(
            (item: any, index: any) =>
              item.category === id && (
                <AssetCard item={item} index={index} key={index} />
              )
          )}
        </div>
      </div>
